    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      <div style={{ position: 'relative', width: size, height: size }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke="#e8ecf0" strokeWidth="5" />
      ...
      <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--slate)', textAlign: 'center' }}>
        {label}
      </span>
    </div>
  )
...
function SummaryRing({ value }: { value: number }) {
  ...
  return (
    <div style={{ position: 'relative', width: size + 60, height: size + 40 }}>
      ...
      <span style={{ position: 'absolute', top: -2, left: '50%', transform: 'translateX(-50%)', fontSize: '0.65rem', color: '#9ca3af', fontWeight: 600 }}>FCP</span>
      <span style={{ position: 'absolute', top: -2, left: 20, fontSize: '0.65rem', color: '#9ca3af', fontWeight: 600 }}>SI</span>
      <span style={{ position: 'absolute', top: 55, right: 0, fontSize: '0.65rem', color: '#9ca3af', fontWeight: 600 }}>LCP</span>
      <span style={{ position: 'absolute', top: 55, left: 0, fontSize: '0.65rem', color: '#9ca3af', fontWeight: 600 }}>CLS</span>
      <span style={{ position: 'absolute', bottom: 22, left: '50%', transform: 'translateX(-50%)', fontSize: '0.65rem', color: '#9ca3af', fontWeight: 600 }}>TBT</span>

      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', margin: '12px auto 0' }}>
        <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke="#e8ecf0" strokeWidth="7" />
      ...
      <div style={{ textAlign: 'center', marginTop: 4 }}>
        <span style={{ fontSize: '0.9rem', fontWeight: 700, color: 'var(--dark)' }}>Performance</span>
      </div>
    </div>
  )
}
...
function DeviceCard({ title, icon, scores, metrics, perfScore }: {
  ...
  return (
    <div style={{
      ...
      flex: 1, minWidth: 320, border: '1px solid #edf0f4',
      boxShadow: '0 4px 24px rgba(0,0,0,0.06)',
    }}>
      {/* Device header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
        {icon}
        <span style={{ color: 'var(--dark)', fontWeight: 700, fontSize: '1.1rem' }}>{title}</span>
      </div>
...
const PhoneIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--slate)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
export default function LighthouseScores() {
  return (
    <section style={{ background: 'var(--light)', padding: '80px 24px', overflow: 'hidden' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          ...
          <h2 style={{
            color: 'var(--dark)', fontSize: 'clamp(1.6rem, 3.5vw, 2.4rem)',
            fontWeight: 800, lineHeight: 1.2, marginBottom: 16,
          }}>
            Green Scores Rank Higher. We Build Green.
          </h2>
          <p style={{ color: 'var(--slate)', fontSize: '1.05rem', lineHeight: 1.65, maxWidth: 680, margin: '0 auto' }}>
