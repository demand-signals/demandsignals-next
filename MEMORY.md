...
**Last updated:** 2026-05-21 23:43 PT — Gaming-PC — Cookie consent kit shipped end-to-end. New reusable widget at `Y:\TOOLS\dsig-cookie-stoplight\` (React + vanilla JS+CSS flavors + README documenting three theming patterns: prop, CSS var, JS global). Canonical reference install on demandsignals.co (commits 5130b67, 783ec9e, 81ff44f). PostHog wired to consent via `opt_out_capturing_by_default: true` + `dsig:consent-changed` event listener (commit 1a9378b) — module-load init preserved per MEMORY 2026-05-19 rule. Consent tiers: red=opt-out, yellow=PostHog on, green=PostHog on + reserved for future marketing scripts. @vercel/analytics removed entirely (1 dep, 43 lockfile lines). AnalyticsTracker first-party cookieless stays unconditional. Side work: demo-sites public landing closed (`6b90e4f` — open directory listing of 15 client demos at apex moved to /demos with noindex; logo-only at /). PAT remediation on demo-sites .git/config: token-bearing URL replaced with clean URL + new `Y:\SCRIPTS\git-credential-dsig.sh` helper sources `$GITHUB_DEMANDSIGNALS_PAT` live from dsig.env. Auto-handoff pipeline used end-to-end this session — all autoMode classifier rules from earlier this week worked silently (every push, every CLI POST). Auto-derived time: Hunter 3h 0m on-clock + Claude 1h 38m line = 4h 38m billable. Tokens: 199k output, 190M cache reads.

---

## SHIPPED 2026-05-21 (cookie consent widget + reusable kit + demo-sites landing close, 7 commits across 2 repos, both Vercel READY)

DSIG-internal session focused on a reusable cookie consent widget plus two side-of-desk closures. Shipped order:

- **demo-sites apex closure (commit 6b90e4f)** — `demandsignals.dev/` was rendering an open directory listing of every active client demo (hach, jach, plan, pasc, ddub, fopa, fach, dmap, capc, claw, gobs, grin, pepl, span, dsig-co + dege). Public anyone-who-guesses-apex could see the whole client roster. Moved to `/demos` (unlinked + noindex/nofollow metadata + robots gates); `/` reduced to centered DSIG logo on white (`/logo.png` copied from demandsignals-next/public). Per-demo subdomains (`hach.demandsignals.dev` etc.) and per-demo paths (`demandsignals.dev/hach` etc.) untouched — every existing direct link still works. Middleware untouched. Catalog UI preserved verbatim from upstream commit 90c60f4 (rebased onto current main; resolved conflict by taking upstream catalog UI on /demos + my logo-only on /).

- **PAT remediation on demo-sites .git/config (commits ad9cb91 + eef3add)** — discovered the demo-sites `.git/config` had `https://github_pat_<TOKEN>@github.com/...` (GITHUB_DEMANDSIGNALS_PAT) embedded in the remote URL — §4 violation (credentials never in any file outside Y:\.credentials\). PAT was rolled a week ago so external-exposure risk was low (verified: never committed; full git history scan clean). Two-step fix: (1) `git remote set-url origin https://github.com/demand-signals/demo-sites.git` (bare URL, no embedded token). Verified push still works via GCM-cached cred. (2) Built `Y:\SCRIPTS\git-credential-dsig.sh` — a git credential helper that sources `$GITHUB_DEMANDSIGNALS_PAT` live from dsig.env on every `get` action for github.com URLs. Wired into demo-sites `.git/config` via `credential.helper = ""` (clear inherited stack) + `credential.helper = Y:/SCRIPTS/git-credential-dsig.sh`. Verified with `GIT_TRACE=1`: my helper called, GCM not called. Token rotation now = edit dsig.env, done. Pattern is reusable: roll same change to other DSIG repos for parity.

- **Cookie consent widget kit at Y:\TOOLS\dsig-cookie-stoplight\** — new top-level Y:\TOOLS\ directory. Reusable kit: `react/CookieStoplight.tsx` (Next.js drop-in, framer-motion, ~360 lines) + `vanilla/cookie-stoplight.{js,css}` (framework-agnostic, no deps, no build, ~250 lines JS + 200 CSS) + README.md (6.7KB documenting install + integration + theming + compliance notes). Visual treatment cloned from `AccessibilityWidget.tsx` — same 40×40 circular bottom-left button, same shadow, sits at `left: 70` to stack with the a11y button. Three colored circles (red/yellow/green = essential/balanced/all) animate up on first visit; collapses to cookie-icon button after choice; click anytime to reopen. Storage = localStorage + cookie (so server-side gates can read it). Event = `dsig:consent-changed` window event for live consent integration.

- **Cookie consent INSTALL on demandsignals.co (commit 5130b67)** — copied `CookieStoplight.tsx` into `src/components/layout/`, mounted in root layout.tsx next to AccessibilityWidget. Build verified locally with placeholder Supabase env vars. Vercel deploy READY. Confirmed widget code shipped in production bundle chunk `0kmab7yhf8d6m.js`.

- **PostHog consent integration (commit 1a9378b)** — wired the widget to the existing PostHog tracker per Hunter directive: red=opt-out, yellow+green=opt-in, green reserved for future marketing scripts. Used PostHog's first-class `opt_out_capturing_by_default: true` flag — library loads but captures zero events until `opt_in_capturing()` fires. **Critically preserves MEMORY 2026-05-19 rule "module-load init only, never lazy in useEffect"** (DOCK rollout cost 5h debugging that pattern). Init still runs at module parse; consent decides capture. Module-load tick reads localStorage and opt-in's immediately for returning balanced/all visitors (no flicker). New `<ConsentWatcher>` component inside PostHogProvider listens for live `dsig:consent-changed` events; toggling consent mid-session takes effect within ms, no page reload. Removed `@vercel/analytics` import + `<Analytics />` mount + `package.json` dep (-1 dep, -43 lockfile lines) per Hunter ("we are not using Vercel for analytics"). AnalyticsTracker (first-party cookieless beacon at src/components/layout/AnalyticsTracker.tsx) stays mounted unconditionally — falls under GDPR/CCPA "strictly necessary" exemption; comment in layout.tsx documents rationale.

- **Cookie panel UX refinements (commits 783ec9e + 81ff44f)** — (1) removed redundant per-tier description block below the three buttons (was duplicating the button labels; user feedback via annotated screenshot). HTML `title=` tooltip on hover + `aria-label` for screen readers carry the descriptions. (2) Added per-site theming: optional `buttonColor` prop, `--cookie-button-color` CSS variable fallback, default to DSIG teal (`#52C9A0`). Only the closed-state button picks up brand color; stoplight circles stay universal red/yellow/green (semantic meaning). Reusable kit synced (same prop, same CSS var, same vanilla `window.DsigCookieStoplightConfig` for non-React installs). DSIG site itself unchanged visually (no prop passed, no var set, falls through to teal default).

- **Commits this session (chronological):** demo-sites: `6b90e4f` (landing close + /demos move) → `ad9cb91` (PAT cleanup auth verification) → `eef3add` (credential helper wiring verification). demandsignals-next: `5130b67` (widget install) → `1a9378b` (PostHog consent + Vercel Analytics removal) → `783ec9e` (description block removal) → `81ff44f` (buttonColor prop + CSS var fallback).

- **Vercel deploys verified READY:** demandsignals-next master @ `81ff44f` (latest), demo-sites main @ `6b90e4f` (latest).

- **Migrations applied:** none this session.

- **Decisions logged this session:**
  - **`Y:\TOOLS\` is the canonical home for reusable cross-project web tooling** — distinct from `Y:\SCRIPTS\` (executable PowerShell/bash helpers) and `Y:\SKILLS\` (skill auto-loaders). The cookie-stoplight kit is the inaugural occupant.
  - **Cookie consent semantic colors are universal across all sites** — red/yellow/green carry meaning, don't rebrand even when sites use other primaries elsewhere. Only the always-visible closed-state button picks up site brand.
  - **Git credential helpers should source tokens live from dsig.env**, not embed in `.git/config` and not cache in Windows Credential Manager. `Y:\SCRIPTS\git-credential-dsig.sh` is the reference helper; pattern is reusable per repo.
  - **PostHog consent gating via `opt_out_capturing_by_default` is the chosen pattern** — preserves the module-load init rule from MEMORY 2026-05-19 while still gating actual capture. Don't ever move init into useEffect.
  - **AnalyticsTracker (first-party cookieless) does not require consent** — server-log-equivalent; falls under "strictly necessary" exemption. Stays mounted for all visitors regardless of cookie choice.

- **Open follow-ups (not done):**
  - Roll the `git-credential-dsig.sh` pattern to other DSIG repos (`demandsignals-next`, `dockside-next`, `southside-mma`, `hangtown-next`) — currently use GCM cache, which goes stale on rotation.
  - PR #1 on demandsignals-next (Brief/Report regenerate + lazy supabaseAdmin) still open from earlier sessions this week.
  - Green-tier marketing scripts (social pixels, ad tracking) — currently no different from yellow. Wire when those are introduced.
