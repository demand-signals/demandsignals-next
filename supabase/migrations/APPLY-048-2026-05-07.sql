-- ── APPLY-048-2026-05-07.sql ───────────────────────────────────────
-- Inlined migration — paste into Supabase web SQL Editor.
--
-- 048: project_notes + project_time_entries EXTENSION + portal_digests
--      + General Support backfill + portal_digest_enabled kill switch.
--
-- project_time_entries already exists from migration 030 with a
-- different shape. This migration EXTENDS it; legacy columns kept.
...
-- /handoff rows don't populate `hours`; relax the NOT NULL + tighten check.
ALTER TABLE project_time_entries
...
ALTER TABLE project_time_entries
  DROP CONSTRAINT IF EXISTS project_time_entries_hunter_minutes_check;
ALTER TABLE project_time_entries
  ADD CONSTRAINT project_time_entries_hunter_minutes_check
    CHECK (hunter_minutes >= 0);

ALTER TABLE project_time_entries
  DROP CONSTRAINT IF EXISTS project_time_entries_claude_minutes_check;
ALTER TABLE project_time_entries
